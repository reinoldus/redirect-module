  ...
  return async function redirectRoute (req, res, next) {
    ...
    const foundSimpleRule = options.simpleRules.find((o) => {
      if (decodedBaseUrl.includes(o.on) && decodedBaseUrl.includes(o.from)) {
        const nextChar = decodedBaseUrl[o.on.length + o.from.length]

        if ((!decodedBaseUrl.includes(o.to) || o.from.includes(o.to)) && nextChar === undefined || nextChar ==='?' || nextChar === '#' || nextChar === '/') {
          return true
        }
      }
      return false
